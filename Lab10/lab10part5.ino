  ...
  if(message.length() > 0){
      ...
      */ 
      ...
      if (message.charAt(0) == '\\' && message.charAt(1) == 't'){
         //requires 6 digits for seconds
        time = message.substring(2).toInt();
        hours = message.substring(2,4).toInt();
        minutes = message.substring(4,6).toInt();
        seconds = message.substring(6,8).toInt();
        timeset= true;
      }
...
//      }
  ...
  if (timeset && time != 0 && count){
    ...
    lcd.clear();
//    String tiempo = hours.toString() + ":" + minutes.toString() + ":" + seconds.toString();
    lcd.print(hours);
    lcd.print(":");
    lcd.print(minutes);
    lcd.print(":");    
